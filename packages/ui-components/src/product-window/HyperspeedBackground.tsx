...
class CarLights {
  ...
  constructor(webgl: App, options: RuntimeHyperspeedOptions, colors: readonly number[] | number, speed: [number, number], fade: THREE.Vector2) {
    this.webgl = webgl;
...
class Road {
  ...
  createPlane(side: number, isRoad: boolean) {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      side: THREE.DoubleSide,
      transparent: true,
      uniforms: Object.assign(uniforms, this.webgl.fogUniforms, (typeof options.distortion === "object" && options.distortion !== null ? options.distortion.uniforms : {}) || {}),
    });
    material.onBeforeCompile = (shader) => {
      shader.vertexShader = shader.vertexShader.replace(
        "#include <getDistortion_vertex>",
...
class App {
  ...
  onPointerDown(ev: PointerEvent) { this.options.onSpeedUp?.(ev as unknown as MouseEvent); this.fovTarget = this.options.fovSpeedUp; this.speedUpTarget = this.options.speedUp; }
  onPointerUp(ev: PointerEvent) { this.options.onSlowDown?.(ev as unknown as MouseEvent); this.fovTarget = this.options.fov; this.speedUpTarget = 0; }
