...
八个首页色块不仅是视觉网格入口，也分别对应八类数据挖掘成果。用户点击任一色块后，该色块应从当前网格位置以 shared-element 方式迅速扩展到全屏，成为独立的数据展示页面；返回时再反向收缩回原色块，形成连续、自然、高级的“窗口打开/关闭”动效，而不是普通跳转。

技术实现上，建议基于 React + Next.js 组织页面与路由，GSAP/Flip 负责色块到全屏的几何过渡动画，ScrollTrigger 管理首页滚动阶段并在进入详情时暂停。色块本体可继续使用 DOM 绝对定位，SVG 只承担辅助线和线稿绘制。详情页内容用模块化配置映射八个成果入口，后续可接入图表、地图、表格和交互分析组件。这样既保留现有网格轨道插值的视觉系统，又能扩展为完整的数据挖掘成果展示平台。

# 首页开窗系统 + 中心正方形修正

## 概要
- 8 个非中心色块升级为 8 个独立的 Next.js 详情路由，但从首页进入时不做普通跳转，而是通过 shared-element 全屏开窗。
- 本阶段 8 个详情页全部先做成纯 `#1E1919` 的全屏 SVG 窗口壳，不放任何内容。
- 同步修正首页终态几何：中心蓝块在最终态必须始终是严格正方形，不再随视口比例变扁或变宽。

## 关键实现
- 路由与状态：
  - 为 8 个彩色块建立 1:1 配置和 slug，`centerCore` 不可点击。
  - 使用 Next.js 独立详情路由 + 拦截/覆盖层方案；从首页打开时保留首页在底层，便于反向关窗回到原块。
  - 在根布局挂一个持久化 transition provider，统一保存首页 panel ref、中心蓝块 ref、当前开窗来源、是否跳过首页 intro、ScrollTrigger 暂停状态。
- 首页交互：
  - 只有桌面端且首页滚动动画到达最终态后，8 个彩色块才可 hover / click。
  - hover 时目标块变为 `#1E1919`，并叠加高动态感处理：轻微放大、前冲、亮边/擦拭式过渡；不改变网格结构，只改变该块的视觉能量和层级。
  - click 时立即冻结首页交互与滚动，暂停 ScrollTrigger，记录被点击色块和中心蓝块的实时几何，再开始路由切换。
- 开窗/关窗动效：
  - 被点击色块从当前网格 rect 直接扩展为全屏黑色窗口。
  - 同时，首页中心蓝块从中心位置出发，移动到左侧轨道中部并缩小为 `45px * 45px`。
  - 窗口落定后，用 GSAP 在距左边 `45px` 处从上到下绘制一根竖线；颜色、透明度、描边参数复用首页现有线条常量。
  - 左侧 `45px` 轨道区域内放置蓝色按钮，颜色与首页中心蓝块完全同源；默认图案是三根横线。
  - 按钮 hover 时，三横线通过路径退让 + 自行车线稿绘制过渡为自行车，不用传统 icon swap，也不引入 MorphSVG 插件。
  - 点击该按钮后完整反向播放：自行车/菜单按钮复位，竖线回收，黑色窗口收回到原彩色块，中心蓝块反向移动并放大回首页中心。
- 详情窗口容器：
  - 详情页用单个全屏 SVG 作为窗口壳：黑底 rect、左侧 45px 轨道、竖线、蓝色按钮、预留内容舞台。
  - 这一阶段内容舞台保持空白；只保留一个未来可挂 `foreignObject` 的占位区域，不引入传统整页 div 容器。
- 中心色块正方形修正：
  - 保留首页初始 intro 大蓝块不变，只修正“终态网格”几何。
  - 现有终态算法改为“中心优先求正方形”：
    - 先按旧比例分别算出原本的中心候选宽度和候选高度。
    - 取两者较小值作为最终 `centerSize`，确保中心块在任何视口下都是严格正方形。
    - 中间列宽直接等于 `centerSize`，中间行高也直接等于 `centerSize`。
    - 剩余宽度按旧侧向权重 `[19, 27, 27, 19]` 分配给四个外围列。
    - 剩余高度按旧上下权重 `[43, 47]` 分配给顶部和底部两行。
  - 所有终态 panel rect、辅助线位置、shared-element 起点、中心蓝块返场位置，都统一读取新的 `centerCoreRect`。
- 直达详情与移动端：
  - 直接访问详情路由或刷新在详情页时，不伪造首页 shared-element 起点；详情页直接以已打开态渲染，仅播放左轨竖线和蓝色按钮的入场微动画。
  - 这种“独立打开态”关闭时，先播放关窗壳层回收，再回到首页最终态；首页 intro 不重播。
  - 移动端保留 SVG 黑窗、左轨、蓝色按钮和返回动画，但不强求 hover 与 Flip 式几何开窗。
...
- 建立统一 insight config：
  ...
  - `themeColor`
  - `isEnabled`
- transition provider 暴露：
  - `registerPanel(panelKey, element)`
  - `registerCenter(element)`
  - `openFromHome(slug)`
  - `closeToHome()`
  - `openStandaloneFallback(slug)`
  - `setHomeSettledOnce()`
- 将现有 `bikePathData` 作为唯一自行车图形来源，并新增一个针对 `45x45` 按钮的缩放包装。

## 测试
- 验证 8 个彩色块在首页最终态都可 hover，并统一过渡到 `#1E1919`。
- 验证任一色块点击后都从自身当前位置扩展到全屏，而不是普通页面切换。
- 验证中心蓝块总是从首页中心移动到左轨中部，并在关闭时精确回到中心。
- 验证左侧竖线始终绘制在距左边 `45px` 处，线条参数与首页一致。
- 验证按钮 hover 时“三横线 → 自行车”过渡成立，点击后能完整触发倒退退出。
- 验证浏览器返回键对“从首页打开的详情页”走同一套反向关窗动画。
- 验证直接打开详情页时不播放伪 shared-element，只播放详情页自身入场。
- 验证不同桌面宽高比下，首页终态中心蓝块始终为严格正方形，不再出现“扁”的情况。
- 验证移动端进入详情时使用简化路径，且不破坏 SVG 窗口视觉。

## 假设
- 这一阶段 8 个详情页全部为空白黑窗，不承载真实数据内容。
- 8 个 slug 先使用中性命名，后续接入真实八类成果时只改配置，不改开窗系统。
- 中心蓝块“正方形”要求针对首页终态网格生效，首页 intro 阶段的大蓝块仍保持当前叙事尺寸。
